...
function getLicenseTier() {
    ...
    for (const [prefix, tier] of Object.entries(TIERS)) {
        if (key.startsWith(prefix) && key.length >= 24)
            return tier;
    }
    return 'none';
}
        ...
        switch (name) {
            ...
            case 'list_meta_campaigns': {
                ...
                if (denied)
                    return fail(denied);
                if (!cfg.hasMeta())
                    return fail('Meta credentials not set. Add META_ADS_ACCESS_TOKEN, META_ADS_ACCOUNT_ID, META_PAGE_ID.');
                const params = {
            ...
            case 'create_meta_campaign': {
                ...
                if (denied)
                    return fail(denied);
                if (!cfg.hasMeta())
                    return fail('Meta credentials not set.');
                if (!cfg.metaPage())
                ...
                const adSetRes = await metaPost(`/${cfg.metaAccount()}/adsets`, {
                    ...
                    targeting: {
                        geo_locations: { countries: args.targeting_countries || ['US'] },
            ...
            case 'enable_meta_campaign': {
                ...
                if (denied)
                    return fail(denied);
                if (!args.campaign_id)
                    return fail('campaign_id required.');
            ...
            case 'list_tiktok_campaigns': {
                ...
                if (denied)
                    return fail(denied);
                if (!cfg.hasTikTok())
                    return fail('TikTok credentials not set. Add TIKTOK_ADS_ACCESS_TOKEN and TIKTOK_ADVERTISER_ID.');
            ...
            case 'create_tiktok_campaign': {
                ...
                const adGroupRes = await tikTokPost('/adgroup/create/', {
                    ...
                    location_ids: [2840], age: args.age_groups || ['AGE_18_24', 'AGE_25_34'],
            ...
            case 'get_tiktok_campaign_stats': {
                ...
                return ok(await tikTokGet('/report/integrated/get/', {
                    ...
                    start_date: args.start_date || fmt(weekAgo),
